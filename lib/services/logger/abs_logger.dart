...
abstract class AbsLogger {
  ...
  CellStyle get _cellStyle => const CellStyle(borderLeft: false, paddingRight: 1);
    ...
    if (enableTableRendering) {
      return '\r\n' +
          Table(
                  ...
                  header: TableSection(cellStyle: _cellStyle, rows: [
                    Row(cells: [Cell('Length'), Cell(list.length.toString())])
                  ]),
                  body: TableSection(
  ...
  void writeLog(LogLevel ll, String name, [Object? value = "", String? tag]) {
    ...
    if (ll.index >= _logLevel!.index &&
        (_tagFilters?.length == 0 || _tagFilters!.contains(tag))) {
      String valueString="";
      //
      if (value != null) {
      ...
      if (splitLog) {
        ...
        while (length > logLength) {
          writeLogLine(
              ...
              logLevelName(ll) +
                  ' ' +
                  logString.substring(startIndex, startIndex + logLength));
        ...
        writeLogLine(
            ...
            logLevelName(ll) +
                ' ' +
                logString.substring(startIndex, startIndex + length));
        //
        writeLogLine(null, "-----------------");
      } else {
